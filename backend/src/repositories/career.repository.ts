...
export const getCareerRoleById=async(roleId:string)=>{
const result=await pool.query(`
SELECT id,title,description,industry,growth_outlook
FROM career_roles
WHERE id=$1
...
export const getCareerRoleSkills=async(roleId:string)=>{
const result=await pool.query(`
...
export const getUserSkillsForCareer=async(userId:string)=>{
const result=await pool.query(`
